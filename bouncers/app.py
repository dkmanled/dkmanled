...
def run_spider(comunas, radius):
    ...
    for comuna in comunas:
        ...
        for place in places:
            ...
            # 2. Buscar en Google Search (Sabueso) para enriquecer redes y contacto
            social_info = {}
            # Siempre intentamos enriquecer si faltan datos clave (redes o teléfono)
            if API_KEYS["SERPER"]:
                search_query = f"{place.get('title')} {nombre_comuna} instagram facebook twitter contacto dueño gerente"
            ...
            try:
                ...
                conn.execute('''
                    INSERT INTO locales (
                        ...
                        justificacion, instagram, facebook, twitter, contacto_nombre, score
                    ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                ''', (
                    ...
                    final_data["contacto_nombre"], final_data["score"]
                ))
                conn.commit()
